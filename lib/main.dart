...
class Planeta {
  ...
  Map<String, dynamic> toMap() {
    return {'id': id, 'nome': nome, 'distancia': distancia, 'tamanho': tamanho, 'apelido': apelido};
  }
}
...
class _PlanetasAppState extends State<PlanetasApp> {
  ...
  void _mostrarFormulario(BuildContext context, {Planeta? planeta}) {
    ...
    final distanciaController = TextEditingController(text: planeta?.distancia.toString() ?? '');
    final tamanhoController = TextEditingController(text: planeta?.tamanho.toString() ?? '');
    final apelidoController = TextEditingController(text: planeta?.apelido ?? '');
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              children: [
                TextField(controller: nomeController, decoration: InputDecoration(labelText: 'Nome do Planeta')),
                TextField(controller: distanciaController, decoration: InputDecoration(labelText: 'Distância do Sol (UA)'), keyboardType: TextInputType.number),
                TextField(controller: tamanhoController, decoration: InputDecoration(labelText: 'Tamanho (km)'), keyboardType: TextInputType.number),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final apelido = apelidoController.text.isEmpty ? null : apelidoController.text;

                if (nome.isEmpty || distancia == null || tamanho == null || distancia <= 0 || tamanho <= 0) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Preencha os campos corretamente')));
                  return;
                }
                ...
                String mensagem;
                if (planeta == null) {
                  sucesso = await DatabaseHelper.instance.insertPlaneta(novoPlaneta) > 0;
                  mensagem = sucesso ? 'Planeta adicionado com sucesso!' : 'Falha ao adicionar planeta.';
                } else {
                  sucesso = await DatabaseHelper.instance.updatePlaneta(novoPlaneta) > 0;
                  mensagem = sucesso ? 'Planeta atualizado com sucesso!' : 'Falha ao atualizar planeta.';
                }
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                trailing: IconButton(
                  ...
                  onPressed: () async {
                    bool sucesso = await DatabaseHelper.instance.deletePlaneta(planeta.id!) > 0;
                    String mensagem = sucesso ? 'Planeta excluído com sucesso!' : 'Falha ao excluir planeta.';
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(mensagem)));
